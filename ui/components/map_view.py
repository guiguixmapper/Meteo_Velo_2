...
def render_map_view(points_gpx, resultats, ascensions, points_eau, infos_soleil,
                    date_depart, heure_arr, fuseau="UTC"):
    # Soleil
    if infos_soleil:
        tz = timezone(fuseau)
        
        # Convertir les heures UTC en heure locale du fuseau
        ...
        <div class="soleil-row">
          ...
          <div class="soleil-item"><div class="s-val">🌅 {ls}</div><div class="s-lbl">Lever</div></div>
          <div class="soleil-item"><div class="s-val">🌇 {cs}</div><div class="s-lbl">Coucher</div></div>
          <div class="soleil-item"><div class="s-val">{hj}h{mj:02d}m</div><div class="s-lbl">Durée du jour</div></div>
        </div>""", unsafe_allow_html=True)
        tz_obj = infos_soleil["lever"].tzinfo
        if date_depart.replace(tzinfo=tz_obj) < infos_soleil["lever"]:
            st.warning(f"⚠️ Départ avant le lever du soleil ({ls}) — prévoyez un éclairage.")
        if heure_arr.replace(tzinfo=tz_obj) > infos_soleil["coucher"]:
            st.warning(f"⚠️ Arrivée après le coucher ({cs}) — prévoyez un éclairage.")
