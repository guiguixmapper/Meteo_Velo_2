...
def analyser_meteo_detaillee(
    ...
    Returns:
        Dict avec stats météo ou None si pas assez de données valides
    """
    try:
...
def calculer_score(
    ...
    Returns:
        Dict avec score final et composantes
        
    Raises:
        ValidationError: Si paramètres invalides
    """
    try:
        if dist_tot < 0 or d_plus < 0:
            raise ValidationError("distances", "Valeurs ne peuvent pas être négatives")
...
def calculer_parcours(
    ...
    Returns:
        Dict contenant:
            - dist_tot (float): Distance totale en mètres
            - d_plus (float): Dénivelé positif cumulé en mètres
            - d_moins (float): Dénivelé négatif cumulé en mètres
            - temps_s (float): Durée totale en secondes
            - cap (float): Dernier cap calculé en degrés
            - checkpoints (list): Points de contrôle générés
            - profil_data (list): Points du profil altimétrique
    """
    checkpoints: list[CheckpointData] = []
    profil_data: list[ProfilePoint] = []
    dist_tot = d_plus = d_moins = temps_s = prochain = cap = 0.0
