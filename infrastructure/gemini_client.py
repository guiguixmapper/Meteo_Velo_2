...
# CORRECTIF : imports remontés en tête de fichier (étaient dans le corps
# de la fonction, ré-exécutés à chaque appel sans raison).
...
"""
        ...
        # CORRECTIF sécurité : on loggue le type d'erreur sans propager l'exception
        # brute qui pourrait contenir la clé API dans sa stacktrace.
        logger.error(f"Erreur Gemini : {type(e).__name__} — {str(e)[:100]}")
        raise RuntimeError(f"Erreur lors de la génération du briefing : {type(e).__name__}") from None
        # Préparer les données
        from core.services.nutrition_service import calculer_hydratation, calculer_glucides
